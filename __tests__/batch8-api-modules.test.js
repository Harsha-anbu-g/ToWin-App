...
import { listKeyholderAsksForMe } from '../src/api/passon';
...
test('listFamilyAlerts reads /family/alerts and keeps the wrapper object', async () => {
...
test('listKeyholderAsksForMe reads /passon/keyholders/asked-of-me', async () => {
  const rows = [{ id: 'k1' }];
  api.get.mockResolvedValue({ data: rows });

  const result = await listKeyholderAsksForMe();
